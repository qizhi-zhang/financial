...
class FCNN():
    def __init__(self, input_dim):
        ...
        self.w1 = tf.Variable(np.random.normal(size=[input_dim, 256])/np.sqrt(256+0.0), dtype='float32', name="param0")
        self.b1 = tf.Variable(np.zeros([1,256]), dtype='float32', name="param1")
        self.w2 = tf.Variable(np.random.normal(size=[256, 128])/np.sqrt(128.0), dtype='float32', name="param2")
        self.b2 = tf.Variable(np.zeros([1, 128]), dtype='float32', name="param3")
        self.w3 = tf.Variable(np.random.normal(size=[128, 64])/np.sqrt(64.0), dtype='float32', name="param4")
        self.b3 = tf.Variable(np.zeros([1, 64]), dtype='float32', name="param5")
        self.w4 = tf.Variable(np.random.normal(size=[64, 32])/np.sqrt(32.0), dtype='float32', name="param6")
        self.b4 = tf.Variable(np.zeros([1, 32]), dtype='float32', name="param7")
        self.w5 = tf.Variable(np.random.normal(size=[32, 1]), dtype='float32', name="param8")
        self.b5 = tf.Variable(np.zeros([1, 1]), dtype='float32', name="param9")
...
def train_and_evaluate(train_datasets, test_datasets, input_dim, epoch, config):
    ...
    for i in range(worker_num):
        ...
        with tf.name_scope("company_{}".format(i)), tf.device(workers[i]):
            batch_size=823
...
if __name__ == '__main__':
    ...
    feature_num = 13
    train_datasets = ["/home/zhangqizhi.zqz/projects/financial/stock/dataset2_10party/train_{}.csv".format(i) for i in range(3)]


    test_datasets = ["/home/zhangqizhi.zqz/projects/financial/stock/dataset2_10party/test.csv"]
    test_datasets_size = [2056]
